...
func main() {
	...
	fmt.Print(")\n")
	if len(os.Args) == 1 {
		fmt.Println("\"flow [대상 데이터팩 디렉토리]\" 형식에 부합하지 않습니다.")
	...
	if err := os.Setenv("INTERNAL_NAMESPACE", "_flow_internal"); err != nil {
		fmt.Println("환경 변수 설정 실패:", err)
		fmt.Println("Press Enter to continue...")
		fmt.Scanln()
		return
	}
	if err := os.Setenv("INTERNAL_PATH", filepath.Join(datapackRootDir, "data", os.Getenv("INTERNAL_NAMESPACE"))); err != nil {
		fmt.Println("환경 변수 설정 실패:", err)
		fmt.Println("Press Enter to continue...")
		fmt.Scanln()
		return
	...
	if err != nil {
		fmt.Println("flow 내부 함수 디렉토리 생성 실패")
		fmt.Println("Press Enter to continue...")
		fmt.Scanln()
		return
	}
	...
	for _, namespace_dir := range namespaces {
		if namespace_dir.Name() == os.Getenv("INTERNAL_NAMESPACE") {
			continue
		}
